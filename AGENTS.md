...
- **Token Budget 压缩**: TokenBudgetCompactor 感知模型 context window，在 80% 阈值触发压缩，双向修复孤立工具对
...
| 模块 | 职责 | 状态 |
|------|------|:----:|
| **cmd/harness9** | 主入口：TTY 自动检测选择 TUI / CLI；初始化 Memory Manager + TokenBudgetCompactor + 首个 Session | ✅ |
...
| **memory** | Context Engineering：Session 接口、Manager（SQLite CRUD）、SQLiteSession（WAL + 事务）、TokenBudgetCompactor（默认，80% 阈值 + 孤立工具对双向修复）、SlidingWindowCompactor（回退方案）、token 估算工具 | ✅ |
...
> **Roadmap（后续方向）**：LLM-based 摘要压缩（P3）、FTS5 全文会话搜索（P3）、TTL 自动过期清理（P3）。
